...
$url	=	"http://cardfight.wikia.com/wiki/Fighter's_Collection_2015";
$html	=	file_get_html($url);

//Obtenemos url de cada carta
$tabla	=	array_slice($html->find('table[class=MsoTableGrid]'), 0, 1);
$links  =   array();
...
//Obtenemos ID de las cartas
foreach($tabla as $table1){
    $i	=	0;
    $k  =   0;
    foreach($table1->find('tr td') as $td){
        if((($i == 0) || ($i == 5)) && (strpos($td->plaintext, 'Card') === false)){
            $id[$k]   =   $td->plaintext;
            //echo $id[$k].'<br>';
            $k++;
        }
...
foreach($links as $link){
    ...
    if($stm = $dbconn->prepare("insert into cards (cardID,name,type_unit,triggers,power,critical,shield,clan,race,effect,text,illustrator,nation,grade_skill) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")){
